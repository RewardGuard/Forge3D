...
export const TOOL_DEFS = [
  ...
  { name: 'add_primitive', description: 'Add a primitive shape (box, sphere, cylinder, cone, pyramid, torus, capsule, plane, tetrahedron, icosahedron) to the 3D scene — with REAL millimetre dimensions per axis via size_mm (e.g. a 157×121×29 mm radiator: {"kind":"box","size_mm":{"w":157,"h":29,"d":121}}). Set negative:true to make a CUTTING shape (group it with a solid to carve; or just use cut_hole).', inputSchema: { type: 'object', properties: { kind: str('primitive kind'), label: str('display label'), color: str('#hex (optional)'), size_mm: { type: 'object', description: '{w,h,d} or {r,h} in real millimetres — exact per-axis sizing (optional)', properties: { w: num('width mm'), h: num('height mm'), d: num('depth mm'), r: num('radius mm') } }, position_mm: { type: 'array', items: num(), description: '[x,y,z] in millimetres (optional)' }, rotation: { type: 'array', items: num(), description: '[x,y,z] radians (optional)' }, negative: bool('cutting shape — carves a hole when grouped with a solid') }, required: ['kind'] } },
  { name: 'cut_hole', description: 'Cut a hole/opening into a solid: creates a negative shape at exact mm size and groups it with the host so the boolean subtraction renders live. Use for ports, vents, windows, screw holes, cable pass-throughs. Host must be a primitive (not an imported STL/model).', inputSchema: { type: 'object', properties: { hostId: str('mesh id of the solid to cut into'), kind: str('box|cylinder — hole shape (cylinder = round hole, default box)'), size_mm: { type: 'object', description: '{w,h,d} or {r,h} millimetres — the hole size', properties: { w: num('width mm'), h: num('height mm'), d: num('depth mm'), r: num('radius mm') } }, position_mm: { type: 'array', items: num(), description: '[x,y,z] millimetres — hole centre (should overlap the host)' }, rotation: { type: 'array', items: num(), description: '[x,y,z] radians (optional)' } }, required: ['hostId', 'size_mm', 'position_mm'] } },
  { name: 'remove_mesh', description: 'Delete objects from the 3D scene by id (works on holes/negatives too). Pass one id or a list.', inputSchema: { type: 'object', properties: { ids: { description: 'mesh id (string) or array of ids' } }, required: ['ids'] } },
  { name: 'save_project', description: 'Save the current project as a REAL .f3d file into the Forge3D project library (Documents/Forge3D/Projects) — no dialogs. Returns the file path so the user can open or share it.', inputSchema: { type: 'object', properties: { name: str('file name without extension, e.g. "water-cooled-laptop"') } } },
  { name: 'export_production', description: 'Export the full production package — firmware code, printable STL, sticker SVG, BOM + build summary — to Documents/Forge3D/Production/<name>/ (USB-ready). Returns the folder path and file list.', inputSchema: { type: 'object', properties: { name: str('package name'), quality: str('low|medium|high STL quality (optional)') } } },
  ...
  { name: 'check_geometry', description: 'Validate the 3D geometry (wheel orientation, floating parts, proportions) and auto-fix what is safe. Pass autofix:false to only report, without moving anything.', inputSchema: { type: 'object', properties: { archetype: str('car|robot|lamp|generic (optional)'), autofix: bool('apply safe fixes (default true)') } } },
  ...
  { name: 'validate_structure', description: 'Engineer-grade physical check: mass, center of mass, support (no floating parts), tip-over stability and interference. REPORT-ONLY by default — nothing is moved. Pass autofix:true to apply safe fixes (avoid on layouts you placed by hand).', inputSchema: { type: 'object', properties: { autofix: bool('apply safe fixes (default false: report only)') } } },
...
const TITLES = {
  ...
  cut_hole: 'Cut a hole into a solid (CSG)', remove_mesh: 'Delete scene objects',
  save_project: 'Save project (.f3d) to library', export_production: 'Export production package (STL/SVG/BOM)',
